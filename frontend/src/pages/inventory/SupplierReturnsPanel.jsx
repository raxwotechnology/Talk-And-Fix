...
import { createSupplierReturn, getSupplierReturns, getSuppliers, getStores } from '../../services/api';

const SupplierReturnsPanel = ({ storeId, products, prefillData, onSuccess }) => {
  ...
  // Apply prefill data if provided (e.g. from GRN history)
  useEffect(() => {
    if (prefillData) {
      if (prefillData.supplierId) setSupplierId(prefillData.supplierId);
  ...
  const fetchSuppliers = async () => {
    try {
      setLoading(true);
      const params = {};
      const targetStore = user?.role === 'admin' ? localStoreId : storeId;
      if (targetStore && targetStore !== 'all') params.storeId = targetStore;
  ...
  }, [storeId, localStoreId, user?.role]);
  ...
  const fetchHistory = async () => {
    try {
      const params = {};
      const targetStore = user?.role === 'admin' ? localStoreId : storeId;
      if (targetStore && targetStore !== 'all' && targetStore !== '') {
        params.storeId = targetStore;
      }
  ...
  useEffect(() => { fetchHistory(); }, [storeId, localStoreId, range.startDate, range.endDate]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const targetStore = user?.role === 'admin' ? localStoreId : storeId;
    if (user?.role === 'admin' && !targetStore) {
    ...
    try {
      ...
      if (user?.role === 'admin') payload.storeId = localStoreId || storeId;
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-card-border p-6 shadow-sm space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div className={user?.role === 'admin' ? '' : 'sm:col-span-1'}>
                ...
                <div className="sm:col-span-5">
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      const opt = productOptions.find(o => o.id === selectedId);
                      updateLine(idx, { 
                        productId: selectedId, 
                        unitCostAtReturn: opt ? opt.cost : '' 
                      });
                    }} 
